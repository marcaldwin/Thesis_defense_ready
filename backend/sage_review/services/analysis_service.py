...
class AnalysisService:
    ...
    def get_basic_text_stats(self, text: str) -> dict[str, int | str]:
        """Return basic manuscript text statistics for display."""
        clean_text = self.normalize_analysis_text(text)
        return {
    ...
    def build_score_summary(self, score_result: dict[str, object]) -> str:
    ...
    def overall_risk_level(self, score: float) -> str:
    ...
    def analyze_streamlit_section(
        self,
        text: str,
        section_name: str | None = None,
    ) -> dict[str, object]:
        """Run the original Streamlit single-section analysis pipeline."""
        clean_text = self.normalize_analysis_text(text)
        if not clean_text:
            raise ValueError("The selected section has no readable text to analyze.")

        stats = self.get_basic_text_stats(clean_text)
        ...
        return {
            ...
            "score_summary": self.build_score_summary(score_result),
    ...
    def analyze_streamlit_full_manuscript(self, text: str) -> dict[str, object]:
        """Run the original Streamlit full-manuscript analysis pipeline."""
        clean_text = self.normalize_analysis_text(text)
        ...
        for section_name, section_text in extracted_sections.items():
            clean_section_text = self.normalize_analysis_text(section_text)
            if not clean_section_text:
                continue

            section_result = self.analyze_streamlit_section(
                clean_section_text,
                section_name,
            )
        ...
        return {
            ...
            "overall_risk_level": self.overall_risk_level(overall_score),
